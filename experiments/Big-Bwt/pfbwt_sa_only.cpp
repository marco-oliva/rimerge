...
static void fwrite_chars_same_suffix_sa(vector<uint32_t> &id2merge,  vector<uint8_t> &char2write, uint32_t *ilist, uint32_t *istart, long &easy_bwts, long &hard_bwts,
                                     int_t suffixLen, FILE *nsuf_file, uint8_t *bwsainfo,long, long& suffissi_stampati, long n_suffs);
         ...
         uint32_t *istart, long dwords) // starting point in ilist for each word and # words
{
  ...
  for(long i=dwords+arg.w+1; i< dsize; i=next ) {
    if (suffissi_stampati > arg.n_suffs) { break; }
...
//      if (d[sa[next]-1]==EndOfWord) { break; }
    ...
    if(arg.n_suffs != 0)
      fwrite_chars_same_suffix_sa(id2merge,char2write,ilist,istart,easy_bwts,hard_bwts,suffixLen,n_suff_file,bwsainfo,psize, suffissi_stampati, arg.n_suffs);
  }
  ...
  delete[] lcp;
  delete[] sa;
  if(arg.n_suffs != 0 or arg.sampledSA!=0) free(bwsainfo);
  if(arg.n_suffs != 0) fclose(n_suff_file);
}
...
static uint8_t *load_bwsa_info(Args &arg, long n)
{
  ...
  if(arg.n_suffs == 0 and arg.sampledSA==0) return NULL;
...
static void fwrite_chars_same_suffix_sa(vector<uint32_t> &id2merge,  vector<uint8_t> &char2write,
                                    ...
                                    int_t suffixLen, FILE *n_suff_file, uint8_t *bwsainfo, long n,
