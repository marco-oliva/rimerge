...
int_t suffixLen, FILE *safile, uint8_t *bwsainfo,long, long& written_suffixes);
...
uint32_t *istart, long dwords) // starting point in ilist for each word and # words
{
    ...
    if(arg.n_suffs != 0) safile = open_aux_file(arg.basename,"nsa","wb");
    ...
    for(long i=dwords+arg.w+1; i< dsize; i=next ) {
        if(written_suffixes > arg.n_suffs) { break; }
        ...
        if(sa[i]==0 || d[sa[i]-1]==EndOfWord) {
            ...
            for(long j=istart[seqid];j<istart[seqid+1];j++) {
                ...
                if(written_suffixes <= arg.n_suffs) { // n SA samples requested
                    if(seqid>0) { // if not the first word in the parse output SA values
                        uint64_t sa = get_myint(bwsainfo,psize,ilist[j]) - suffixLen;
                        if(fwrite(&written_suffixes,SABYTES,1,safile)!=1) die("SA write error 0");
                    ...
                    else
                    {
                        ...
                        if(fwrite(&sa,SABYTES,1,safile)!=1) die("SA write error 0");
                        written_suffixes += 1;
                    } // the first word in the parse is the 2nd lex smaller and does not correspond to a SA entry
                }
        ...
        fwrite_chars_same_suffix_sa(id2merge,char2write,ilist,istart,fbwt,easy_bwts,hard_bwts,suffixLen,safile,bwsainfo,psize, written_suffixes);
    }
    ...
    delete[] lcp;
    delete[] sa;
    if(arg.n_suffs != 0) free(bwsainfo);
    if(arg.n_suffs != 0) fclose(safile);
...
void print_help(char** argv, Args &args) {
    ...
    cout << "  Options: " << endl
         ...
         << "\t-N  \tcompute first N elements of the suffix array" << endl;
    exit(1);
}
...
void parseArgs( int argc, char** argv, Args& arg ) {
    ...
    string sarg;
    while ((c = getopt( argc, argv, "t:N:w:h") ) != -1) {
        switch(c) {
        case 'w':
            sarg.assign( optarg );
...
int_t suffixLen, FILE *safile, uint8_t *bwsainfo, long n, long& written_suffixes)
{
    ...
    if(numwords==1) {
        ...
        for(long j=istart[s];j<istart[s+1];j++) {
            ...
            if(fwrite(&written_suffixes,SABYTES,1,safile)!=1) die("SA write error 1");
        ...
        while(heap.size()>0) {
            ...
            if(fwrite(&written_suffixes,SABYTES,1,safile)!=1) die("SA write error 2");
