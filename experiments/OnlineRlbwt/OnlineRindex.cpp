...
#include "../../include/rimerge/malloc_count.h"
...
//    }
    ...
    {
        sequences++;
        auto t_inter = std::chrono::high_resolution_clock::now();
        double sec = std::chrono::duration_cast<std::chrono::seconds>(t_inter - t_prev).count();
        t_prev = t_inter;
        std::cout << sequences << "\t\t" << sec << "s" << std::endl;
